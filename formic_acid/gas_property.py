...
_component_params = {
    ...
    "CH2O2": {
        ...
        "visc_d_phase_comp": {"Vap": ChapmanEnskogLennardJones},
        "therm_cond_phase_comp": {"Vap": Eucken},
        "elemental_composition": {"C": 1, "H": 2, "O": 2},
        "enth_mol_ig_comp": NIST,
        "entr_mol_ig_comp": NIST,
        "cp_mol_ig_comp": NIST,
        "parameter_data": {
            "mw": (0.046, pyunits.kg / pyunits.mol),
            "pressure_crit": (5.81e6, pyunits.Pa),
            "temperature_crit": (588.0, pyunits.K),
            "omega": 0.239, #not sure, use CO2 value
            "cp_mol_ig_comp_coeff": {
                "A": 45.68,
                "B": 0.0,
                "C": 0.0,
            ...
            "lennard_jones_sigma": (4.04, pyunits.angstrom),
            "lennard_jones_epsilon_reduced": (183.9, pyunits.K),
            "f_int_eucken": 1,
        },
    },
    "O2": {
...
def get_prop(components=None, phases="Vap", eos=EosType.PR, scaled=False):
    ...
    for comp in components:
        ...
        if comp == "D2O":
            c["D2O"]["visc_d_phase_comp"] = copy.deepcopy(
                {p: _water_visc_d[p] for p in phases}
            )
            c["D2O"]["therm_cond_phase_comp"] = copy.deepcopy(
                {p: _water_therm_cond[p] for p in phases}
            )
    for k in phases:
        if eos == EosType.PR:
