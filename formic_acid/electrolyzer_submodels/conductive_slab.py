...
class SocConductiveSlabData(UnitModelBlockData):
    ...
    CONFIG.declare(
        "has_external_cooling",
        ConfigValue(
            domain=Bool,
            default=False,
            description="If True, calculate external cooling heat flux and set outside wall temperature",
        ),
    )
    ...
    def build(self):
        ...
        if self.config.has_external_cooling:
            self.temperature_wall_outside = pyo.Param(initialize=300.0, mutable=True, doc="External wall temperature")
        ...
        if self.config.has_external_cooling:
            @self.Constraint(tset, iznodes)
            def external_wall_temperature_eqn(b, t, iz):
                return b.temperature_x1[t, iz] == b.temperature_wall_outside
