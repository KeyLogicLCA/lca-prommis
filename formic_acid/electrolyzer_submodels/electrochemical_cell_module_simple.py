...
from . import ElectrochemicalCell
from . import common
...
@declare_process_block_class("ElectrochemicalCellModuleSimple")
class ElectrochemicalCellModuleSimpleData(UnitModelBlockData):
    CONFIG = UnitModelBlockData.CONFIG()
    # CONFIG = ConfigBlock()
    CONFIG.declare(
        "electrochemical_cell_config",
        ConfigBlock(
            implicit=True,
            description="Config block for the ElectrochemicalCell",
        ),
    )
    CONFIG.declare(
    ...
    CONFIG.declare(
        "acid_property_package",
        ConfigValue(
            default=None,
            domain=is_physical_parameter_block,
            description="Property package to use for acid-side stream",
            doc="""Property parameter object used to define property
                    calculations for the acid-side stream,
                    **default** - None.
                    **Valid values:** {
                    **PhysicalParameterObject** - a PhysicalParameterBlock object.}""",
        ),
    ...
    CONFIG.declare(
        "acid_property_package_args",
        ConfigBlock(
            implicit=True,
            description="Arguments to use for constructing property package "
            "of the acid-side stream",
            doc="""A ConfigBlock with arguments to be passed to the property
                    block associated with the acid-side stream,
                    **default** - None.
                    **Valid values:** {
                    see property package for documentation.}""",
        ),
    ...
    def build(self):
        ...
        self.electrochemical_cell = ElectrochemicalCell(
            **self.config.electrochemical_cell_config
        )
        ...
        for side in ["fuel", "acid", "oxygen"]:
            ...
            side_comps = getattr(self.electrochemical_cell, f"{side}_component_list")
            ...
            for direction in ["in", "out"]:
                ...
                port = getattr(self.electrochemical_cell, port_name)
                setattr(
                    self,
        ...
        def electrical_work(b, t):
            return b.electrochemical_cell.electrical_work[t] * b.number_cells

        @self.Expression(tset)
        def heat_loss(b, t):
            return b.electrochemical_cell.external_cooling_duty[t] * b.number_cells

        self.potential_cell = pyo.Reference(self.electrochemical_cell.potential)
        ...
        def total_current_eqn(b, t):
            return (
                b.total_current[t]
                == b.electrochemical_cell.total_current[t] * b.number_cells
            )

    def initialize_build(
        self,
        state_args_fuel=None,
        state_args_acid=None,
        ...
        for side, state_args in zip(
            ["fuel", "acid", "oxygen"],
            [state_args_fuel, state_args_acid, state_args_oxygen],
        ):
            props_in = getattr(self, side + "_properties_in")
            inlet = getattr(self.electrochemical_cell, side + "_inlet")
            comps = getattr(self.electrochemical_cell, side + "_component_list")
        ...
        self.electrochemical_cell.initialize_build(
            outlvl=idaeslog.INFO_LOW,
            solver=solver,
            optarg=optarg,
        ...
        self.acid_properties_in.release_state(flags=flags["acid"], outlvl=outlvl)
    ...
    def calculate_scaling_factors(self):
        ...
        for side in ["fuel", "acid", "oxygen"]:
            comps = getattr(self.electrochemical_cell, f"{side}_component_list")
            channel = getattr(self.electrochemical_cell, f"{side}_channel")
            for direction in ["in", "out"]:
                props = getattr(self, f"{side}_properties_{direction}")
                port_name = f"{side}_{direction}let"  # e.g., fuel_inlet, oxygen_outlet
                ...
                for t in self.flowsheet().time:
                    sT = gsf(props[t].temperature, default=0.01) # in case scaling factor not set yet
                    cst(temperature_eqn[t], sT)
                    sP = gsf(props[t].pressure, default=1e-5) # in case scaling factor not set yet
                    cst(pressure_eqn[t], sP)
                    sflow = gsf(props[t].flow_mol, default=1.0/5000)
                    print("------sflow---", sflow)
                    ...
                    for j in comps:
                        sx = gsf(props[t].mole_frac_comp[j], default=10.0)
        ...
        self.electrochemical_cell.recursive_scaling()
        for t in self.flowsheet().time:
            sf_tot_current = gsf(self.electrochemical_cell.total_current[t])
