...
#################################################################################
"""
Model for nonisothermal (single) electrolytic cell. Because
...
This model has at least seven submodels:
    - Flue gas/formic acit/oxygen channels
...
Ports:
    * ``fuel/acid/oxygen_inlet``: Lifts channel level inlet variables to cell level.
    ...
    * ``fuel/acid/oxygen_outlet``: Lifts channel level outlet variables to cell level.
...
__author__ = "John Eslick, Douglas Allan, Jinliang Ma"
...
from . import SocChannel, PorousConductiveSlab, SocTriplePhaseBoundary, SocTriplePhaseAcidBoundary, SocConductiveSlab, SocContactResistor
from . import common
...
@declare_process_block_class("ElectrochemicalCell")
class ElectrochemicalCellData(UnitModelBlockData):
    ...
    # added for formic acid channel
    CONFIG.declare(
        "acid_component_list",
        ConfigValue(
            domain=common._SubsetOf(_gas_species_list),
            default=None,
            description="List of components in formic acid product stream",
        ),
    )
    CONFIG.declare(
        ...
        ConfigValue(
            domain=common._SubsetOf(_gas_species_list),
            default=None,
            description="List of components in combining fuel and acid channel",
        ),
    )
    CONFIG.declare(
    ...
    def build(self):
        ...
        if self.config.fuel_component_list is None:
            fuel_comp_list = ["N2", "CO2", "O2", "D2O", "Ar"]
        else:
            fuel_comp_list = self.config.fuel_component_list
        if self.config.acid_component_list is None:
            acid_comp_list = ["CH2O2", "H2O"]
        else:
            acid_comp_list = self.config.acid_component_list
        if self.config.oxygen_component_list is None:
            oxygen_comp_list = ["O2", "H2O"]
        else:
            oxygen_comp_list = self.config.oxygen_component_list
        if self.config.combined_component_list is None:
            combined_comp_list = ["CO2", "N2", "H2O", "D2O", "CH2O2", "O2", "Ar"]
        else:
            combined_comp_list = self.config.combined_component_list
        if self.config.fuel_triple_phase_boundary_stoich_dict is None:
            fuel_tpb_stoich = {"CH2O2": -0.5, "CO2": 0.5, "H^+": 1.0, "e^-": 1.0}
        else:
        ...
        if self.config.oxygen_triple_phase_boundary_stoich_dict is None:
            oxygen_tpb_stoich = {"O2": -0.25, "H^+": -1.0, "e^-": -1.0, "H2O": 0.5}
        else:
        ...
        self.acid_component_list = pyo.Set(
            initialize=acid_comp_list,
            ordered=True,
            doc="Set of all gas-phase components present on acid product side of cell",
        )
        self.combined_component_list = pyo.Set(
            initialize=combined_comp_list,
            ordered=True,
            doc="Set of all gas-phase components present at fuel triple phase boundary",
        )
        ...
        self.acid_channel = SocChannel(
            ...
            length_y=self.length_y,
            temperature_z=self.temperature_z,
            opposite_flow=False,  # Assume the flow direction is the same as the flue gas
            below_electrode=False,  #this make the flow similar to oxygen channel with triple phase bounary below the channel
            component_list=self.acid_component_list,
            include_temperature_x_thermo=include_temp_x_thermo,
        )
        self.oxygen_channel = SocChannel(
            has_holdup=has_gas_holdup,
            dynamic=has_gas_holdup and dynamic,
            control_volume_zfaces=self.config.control_volume_zfaces,
            length_z=self.length_z,
        ...
        if self.config.include_contact_resistance:  # currently it is assumed this is False
        ...
        if self.config.thin_fuel_electrode:  # Currently assume False
            ...
            self.fuel_electrode = PorousConductiveSlab(
        ...
        if self.config.thin_oxygen_electrode:  # Currently assume False
            ...
            self.oxygen_electrode = SocContactResistor(
                has_holdup=False,
                dynamic=False,
                control_volume_zfaces=self.config.control_volume_zfaces,
                length_z=self.length_z,
            ...
            self.oxygen_electrode = PorousConductiveSlab(
                has_holdup=has_holdup,
                has_gas_holdup=has_gas_holdup,
                dynamic=dynamic,
                control_volume_zfaces=self.config.control_volume_zfaces,
        ...
        # it seems that the fuel triple phase boundary model needs to be revised to consider species from/to
        # both fuel electrode and acid channel
        self.fuel_triple_phase_boundary = SocTriplePhaseAcidBoundary(
            has_holdup=False,
            dynamic=False,
            control_volume_zfaces=self.config.control_volume_zfaces,
            length_z=self.length_z,
            length_y=self.length_y,
            component_list_fuel=self.fuel_component_list,
            component_list_acid=self.acid_component_list,
            component_list=self.combined_component_list,
            reaction_stoichiometry=fuel_tpb_stoich,  # user specified through config
            inert_species=self.config.inert_fuel_species_triple_phase_boundary,  # user specified through config but not sure how this is used
            current_density=self.current_density,
            temperature_z=self.temperature_z,
            temperature_deviation_x=fuel_electrode_temperature_deviation_x1, # assume this is the same as the acid channel x0
            heat_flux_x0=self.fuel_electrode.heat_flux_x1,
            heat_flux_x1=self.acid_channel.heat_flux_x0,
            # not sure if heat_fux_x1 should be assigned as the acid channel heat_flux_x0, also see initialize_build function
            conc_mol_comp_ref_fuel=self.fuel_channel.conc_mol_comp,
            conc_mol_comp_ref_acid=self.acid_channel.conc_mol_comp,
            conc_mol_comp_deviation_x_fuel=fuel_electrode_conc_mol_comp_deviation_x1,
            conc_mol_comp_deviation_x_acid=self.acid_channel.conc_mol_comp_deviation_x0,
            material_flux_x_fuel=fuel_electrode_material_flux_x1,
            material_flux_x_acid=self.acid_channel.material_flux_x0,
            include_temperature_x_thermo=include_temp_x_thermo,
            below_electrolyte=True,
            voltage_drop_custom=self.config.voltage_drop_custom,
        )
        self.oxygen_triple_phase_boundary = SocTriplePhaseBoundary(
            has_holdup=False,
            dynamic=False,
            control_volume_zfaces=self.config.control_volume_zfaces,
            length_z=self.length_z,
        ...
        if self.config.thin_electrolyte:
            ...
            self.electrolyte = SocContactResistor(
                has_holdup=False,
                dynamic=False,
                control_volume_zfaces=self.config.control_volume_zfaces,
                length_z=self.length_z,
            ...
            self.electrolyte = SocConductiveSlab(
                ...
                temperature_deviation_x0=self.acid_channel.temperature_deviation_x1,
                temperature_deviation_x1=oxygen_electrode_temperature_deviation_x0,
                #heat_flux_x0=self.fuel_triple_phase_boundary.heat_flux_x1,
                heat_flux_x0=self.acid_channel.heat_flux_x1,
                heat_flux_x1=self.oxygen_triple_phase_boundary.heat_flux_x0,
                include_temperature_x_thermo=include_temp_x_thermo,
                voltage_drop_custom=self.config.voltage_drop_custom,
            )

        # add acid channel inlet and outlet
        self.state_vars = {"flow_mol", "mole_frac_comp", "temperature", "pressure"}
        for chan, alias in zip(
            [self.fuel_channel, self.acid_channel, self.oxygen_channel], ["fuel", "acid", "oxygen"]
        ):
            setattr(
                self,
                alias + "_inlet",
        ...
        if self.config.flux_through_interconnect:
            if self.config.thin_interconnect:
                ...
                self.interconnect = SocContactResistor(
                    has_holdup=False,
                    dynamic=False,
                    control_volume_zfaces=self.config.control_volume_zfaces,
                    length_z=self.length_z,
                ...
                self.interconnect = SocConductiveSlab(
                    has_holdup=has_holdup,
                    dynamic=dynamic,
                    control_volume_zfaces=self.config.control_volume_zfaces,
        ...
        @self.Constraint(tset, iznodes)
        def acid_temperature_x0_eqn(b, t, iz):
            return (
                b.acid_channel.temperature_deviation_x0[t, iz]
                == b.fuel_triple_phase_boundary.temperature_deviation_x[t, iz]
            )
        ...
        @self.Expression(tset)
        def external_cooling_duty(b, t):
            return (sum(b.xface_area[iz]*(b.interconnect.heat_flux_x1[t,iz]
                        -b.fuel_channel.heat_flux_x0[t,iz]) for iz in b.iznodes )
            )
    ...
    def initialize_build(
        ...
        for chan, comps in zip(
            ["fuel", "acid", "oxygen"], [self.fuel_component_list, self.acid_component_list, self.oxygen_component_list]
        ):
            pname = chan + "_inlet"
        ...
        init_log.info_high("Initializing Acid Channel")
        self.acid_channel.material_flux_x0.fix(0)
        self.acid_channel.heat_flux_x0.fix(0)
        self.acid_channel.heat_flux_x1.fix(0)
        self.acid_channel.initialize_build(
            outlvl=outlvl,
            solver=solver,
            optarg=optarg,
        )
        self.acid_channel.material_flux_x0.unfix()
        self.acid_channel.heat_flux_x0.unfix()
        self.acid_channel.heat_flux_x1.unfix()
        ...
        self.fuel_triple_phase_boundary.conc_mol_comp_deviation_x_fuel.fix(0)
        self.fuel_triple_phase_boundary.conc_mol_comp_deviation_x_acid.fix(0)
        #self.fuel_triple_phase_boundary.conc_mol_comp_ref.fix()
        self.fuel_triple_phase_boundary.conc_mol_comp_ref_fuel.fix()
        self.fuel_triple_phase_boundary.conc_mol_comp_ref_acid.fix()
        self.fuel_triple_phase_boundary.heat_flux_x1.fix(0)
        # I assume heat_flux_x0 is calculated
        ...
        self.fuel_triple_phase_boundary.conc_mol_comp_deviation_x_fuel.unfix()
        self.fuel_triple_phase_boundary.conc_mol_comp_deviation_x_acid.unfix()
        #self.fuel_triple_phase_boundary.conc_mol_comp_ref.unfix()
        self.fuel_triple_phase_boundary.conc_mol_comp_ref_fuel.unfix()
        self.fuel_triple_phase_boundary.conc_mol_comp_ref_acid.unfix()
        ...
        #self.current_density.fix(-100)
        #self.current_density.unfix()
        #self.potential.fix(2.5)
    ...
    def model_check(self, steady_state=True):
        ...
        for t in self.flowsheet().config.time:
            for element in _element_list:
                ...
                sum_out = 0
                for chan, comps in zip(
                    [self.fuel_channel, self.acid_channel, self.oxygen_channel],
                    [self.fuel_component_list, self.acid_component_list, self.oxygen_component_list],
                ):
                    sum_in += sum(
            ...
            enth_in = (
                ...
                * self.fuel_channel.flow_mol_inlet[t]
                + self.acid_channel.enth_mol_inlet[t]
                * self.acid_channel.flow_mol_inlet[t]
            ...
            enth_out = (
                ...
                * self.fuel_channel.flow_mol_outlet[t]
                + self.acid_channel.enth_mol_outlet[t]
                * self.acid_channel.flow_mol_outlet[t]
    ...
    def recursive_scaling(self):
        ...
        sdf(self.current_density, 5e-3)  # original 5e-4
        ...
        sdf(self.acid_inlet.temperature, 1e-2)
        sdf(self.acid_inlet.pressure, 1e-4)
        sdf(self.acid_inlet.flow_mol, 1e5)
        sdf(self.acid_inlet.mole_frac_comp, sy_def)
        ...
        for t in self.flowsheet().time:
            ...
            #add acid inlet
            sy_in_acid = {}
            for j in self.acid_component_list:
                sy_in_acid[j] = gsf(self.acid_inlet.mole_frac_comp[t, j], sy_def)
            sy_in_oxygen = {}
            ...
            for iz in self.iznodes:
                ...
                # add acid channel stuff
                for j in self.acid_component_list:
                    if j in self.config.inert_fuel_species_triple_phase_boundary:
                        s_flux_j = sy_in_acid[j] * s_inert_flux
                    else:
                        s_flux_j = sy_in_acid[j] * s_react_flux
                    # s_flux_j = sy_in_oxygen[j]*s_mat_flux / max(abs(self.oxygen_triple_phase_boundary.tpb_stoich[j]),0.25)
                    ssf(self.acid_channel.material_flux_x0[t, iz, j], s_flux_j)
                for j in self.oxygen_component_list:
