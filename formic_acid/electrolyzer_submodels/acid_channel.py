...
#################################################################################
"""
Model for channel in ElectrolyzerCell. If opposite_flow is false, material flows from
...
assumed that the fluid flowing through the channel is liquid (water + formic acid).
The density of formic acid is 1220 kg/m^3 and density of water is 1000 kg/m^3, corresponding
to mole densities of 1220/0.04603=26504 mol/m^3 and 1000/0.018=55555.6 mol/m^3.
The mixture density or mole valume is linear function of mole fraction.
...
For formic acid electrolyzer, the channel is below electrode.
...
The properties of liquid formic acid and water mixture is calculated as follows:
Formic acid standard heat of formation (liquid) -425090 J/mol Cp=99.04 J/mol-K (NIST WebBook)
Water standard heat of formation (liquid) -285830 J/mol Cp=4184 J/kg-K or Cp=75.3 J/mol-K
For internal energy U=H-pV here p=101325, V=0.018/1000 for water and 0.04603/1220 for formic acid
U = H-1.82 J/mol for water and U=H-3.82 J/mol for formic acid
"""
...
class AcidChannelData(UnitModelBlockData):
    ...
    def build(self):
        ...
        # parameters for liquid properties
        self.temperature_ref = Param(initialize=298.15, doc="reference temperature", units=pyo.units.K)

        den_mol_std_const = {"H2O": 1000/0.018, "CH2O2": 1220/0.04603}
        self.den_mol_std = pyo.Param(
             comps,
             initialize=den_mol_std_const,
             doc="molar density of component",
             units=pyo.units.mol / pyo.units.m**3,
             )

        enth_mol_std_const = {"H2O": -285830, "CH2O2": -425090}
        self.enth_mol_std = pyo.Param(
             comps,
             initialize=enth_mol_std_const,
             doc="standard heat of formation of component",
             units=pyo.units.J / pyo.units.mol,
             )

        int_energy_mol_std_const = {"H2O": -285830 - 1.82, "CH2O2": -425090 - 3.82 }
        self.int_energy_mol_std = pyo.Param(
             comps,
             initialize=int_energy_mol_std_const,
             doc="standard internal energy",
             units=pyo.units.J / pyo.units.mol,
             )


        cp_mol_std_const = {"H2O": 75.3, "CH2O2": 99.04}
        self.cp_mol_std = pyo.Param(
             comps,
             initialize=cp_mol_std_const,
             doc="heat capacity of component",
        ...
        # JM, liquid density calculated from mole fraction
        @self.Expression(tset, iznodes)
        def vol_mol(b, t, iz):
            return sum(b.mole_frac_comp[t, iz, j]/b.den_mol_std[j] for j in b.component_list)

        # JM, liquid density at inlet calculated from mole fraction
        @self.Expression(tset)
        def vol_mol_inlet(b, t):
            return sum(b.mole_frac_comp_inlet[t, j]/b.den_mol_std[j] for j in b.component_list)
        ...
        def enth_mol_inlet(b, t):
            return sum(
                b.mole_frac_comp_inlet[t, i]*(b.enth_mol_std[i] + b.cp_mol_std[i]*(b.temperature_inlet[t] - b.temperature_ref))
                for i in b.component_list
            )

        # JM, currently assume constant diffusivity of 1.5e-9
        # Vincent van Beusekom, "Electrochemical Reduction of Carbon Dioxide-Modeling the electrochemical conversion of CO2 to Formic Acid under high pressure"
        ...
        # JM, calculate based on vol_mol
        @self.Constraint(tset, iznodes, comps)
        def conc_mol_comp_eqn(b, t, iz, i):
            return (
                b.conc_mol_comp[t, iz, i] * b.vol_mol[t, iz]
                ==  b.mole_frac_comp[t, iz, i]
            )

        # JM, calculate based on mole fraction
        @self.Constraint(tset, iznodes)
        def enth_mol_eqn(b, t, iz):
            return b.enth_mol[t, iz] == sum(
                b.mole_frac_comp[t, iz, i]
                * (b.enth_mol_std[i] + b.cp_mol_std[i]
                * (b.temperature[t, iz] - b.temerature_ref))
                for i in b.component_list
            )
        ...
        if self.config.has_holdup:
            ...
            def int_energy_mol_eqn(b, t, iz):
                ...
                b.mole_frac_comp[t, iz, i]
                * (b.int_energy_mol_std[i] + b.cp_mol_std[i]
                * (b.temperature[t, iz] - b.temerature_ref))
                for i in b.component_list
            )
        ...
        # JM, calculate enthalpy based on liquid
        @self.Constraint(tset, iznodes)
        def energy_balance_eqn(b, t, iz):
            return (
                ...
                * (b.material_flux_z_enth[t, iz] - b.material_flux_z_enth[t, iz + 1])
                + b.xface_area[iz]
                * sum(
                    b.material_flux_x0[t, iz, i]*(b.enth_mol_std[i] + b.cp_mol_std[i]
                    * (b.temperature_x0[t, iz] - b.temerature_ref))
                    - b.material_flux_x1[t, iz, i]*(b.enth_mol_std[i] + b.cp_mol_std[i]
                    * (b.temperature_x1[t, iz] - b.temerature_ref))
                    for i in comps
                )
        ...
        # JM, enthalpy calculation based on liquid property
        @self.Constraint(tset)
        def temperature_outlet_eqn(b, t):
            return b.enth_mol_outlet[t] == sum(
                b.mole_frac_comp_outlet[t, i] * (b.enth_mol_std[i] + b.cp_mol_std[i]*(b.temperature_outlet[t] - b.temperature_ref))
                for i in comps
            )
