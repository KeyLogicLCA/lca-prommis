...
def _init_solve_block(blk, solver, log):
    ...
    with idaeslog.solver_log(log, idaeslog.INFO_LOW) as slc:
...
def _interpolate_2D(
    ...
    if not derivative:
        ...
        return lambf * phi_func(icu) + (1 - lambf) * phi_func(icd)
    else:
...
_all_species_list.extend(["Vac", "O^2-", "H^+", "e^-"])
_element_dict = {
    "Ar": {"Ar": 1},
    "H": {"CH4": 4, "C2H4": 4, "C2H6": 6, "C3H8": 8, "H2": 2, "H2O": 2, "D2O": 2, "H2S": 2, "CH2O2": 2},
    "O": {"CO": 1, "CO2": 2, "H2O": 1, "D2O": 1, "NO": 1, "N2O": 1, "O2": 2, "SO2": 2, "CH2O2": 2},
    "C": {"CH4": 1, "CO": 1, "CO2": 1, "C2H4": 4, "C2H6": 2, "C3H8": 3, "CH2O2": 1},
    "N": {"NO": 1, "N2": 2, "N2O": 2},
    "S": {"H2S": 1, "SO2": 1},
}
...
bin_diff_sigma = {
    ...
    "O2": 3.467,
    "SO2": 4.112,
    "CH2O2": 4.04, #https://doi.org/10.1002/kin.21341
}
bin_diff_epsok = {
    "Ar": 93.3,
    "Air": 78.6,
    ...
    "O2": 106.7,
    "SO2": 335.4,
    "CH2O2": 183.9, #https://doi.org/10.1002/kin.21341
}
bin_diff_M = {
    "Ar": 39.948,
    "Air": 28.96,
