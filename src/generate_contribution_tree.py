...
# -*- coding: utf-8 -*-
#
# generate_contributions_tree.py
#
###############################################################################
# DEPENDENCIES
###############################################################################
...
4.  A boolean on whether save results to CSV file.

Read impact category.

For each impact category, extract the results (combination tree) and export
the results to an excel file holding the name of the impact category.
...
# FUNCTIONS
###############################################################################
def generate_contribution_tree(result,
                               max_expand_levels,
                               max_expand_nodes,
                               download_results):
    """
    This function generates the results contribution tree for each impact
    category.

    The function returns a data frame for each impact category within the
    ...
    max_expand_levels : int
        This represents the number of steps away to take from the main product.
    max_expand_nodes : int
        This represents the number of child nodes to expand for each level
        (e.g., electricity or natural gas).
    download_results : bool

    Returns
    -------
    pandas.DataFrame
        A data frame for each impact category within the impact assessment
        method used.
    """
    ...
    # this is defined here and used below to refer to the row of the impact
...
def expand(node: utree.Node,
           level: int,
           max_expand_levels: int,
           max_expand_nodes: int):
    """Helper method to expand the ``utree`` node.

    Code was adapted from GreenDelta, `here <https://greendelta.github.io/openLCA-ApiDoc/results/impacts/upstream_trees.html>`_.
    ...
    if level < max_expand_levels:
        for c in node.childs[0:max_expand_nodes]:
            results.extend(
                expand(c, level + 1, max_expand_levels, max_expand_nodes)
            )
    return results
