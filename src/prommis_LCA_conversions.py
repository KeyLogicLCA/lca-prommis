...
try:
    ...
    print(
        "Warning: pubchempy and/or pymatgen not available. "
        "Molecular weight conversions will be disabled."
    )
...
This module converts PrOMMiS flowsheet data from various units to LCA-relevant
units for Life Cycle Assessment analysis.
It handles complex unit conversions, mass/mole fractions, and provides automatic molecular weight lookups for chemical compounds.
...
-   :func:`main`: Executes the complete conversion workflow and saves results.
-   :func:`convert_flows_to_lca_units`: Core conversion function with
    comprehensive unit handling.
-   :func:`get_molar_mass`: Retrieves molecular weights from PubChem database.
-   :func:`parse_unit_to_pyomo`: Converts unit strings to Pyomo unit expressions
-   :func:`get_unit`: Resolves individual unit strings to Pyomo units
...
'''dict : Map units to display names.'''
...
def main():
    ...
    This function reads the raw LCA DataFrame, converts all flows to
    LCA-relevant units, and saves the results to a new CSV file. It serves as
    the main entry point for the unit conversion process.
    ...
    -   Uses default conversion parameters (1 hour period, moles to kg, water
        in m3)
...
def convert_flows_to_lca_units(df, hours=1, mol_to_kg=True, water_unit='m3'):
    """
    Convert PrOMMiS flowsheet flows to LCA-relevant units using Pyomo's unit
    system.

    This function processes a DataFrame containing PrOMMiS flowsheet data and
    converts all flows to consistent LCA units (kg, L, kWh, MJ, mol).
    It handles various unit formats, mass/mole fractions, concentrations, and
    special cases like embedded values in unit strings.
    ...
    hours : float, optional
        Time period for conversion (default: 1). Multiplies all amounts by this
        value to convert from flow rates to total amounts.
    mol_to_kg : bool, optional
        Whether to convert molar amounts to mass using molecular weights.
        Default to true.
        Requires internet connection for PubChem lookups.
    water_unit : string, optional
        What unit to use for water flows. Can be 'm3', 'L', or 'kg'.
        Defaults to 'm3'.
    ...
    -   Automatically determines target units based on flow category:
        ...
        -   Water/wastewater: kg (if concentration given), L (if volume), or
            specified unit
        -   Electricity: kWh
        -   Heat: MJ
    ...
    -   If using mass/mole fractions, the first value should be the total
        stream amount,and the second value should be the composition of the
        individual flow
    ...
    for idx, row in df.iterrows():
        ...
        # This section ensures robust handling of various input formats and
        # edge cases
        try:
        ...
        try:
            if (pd.isna(row['Value 2'])
                    or row['Value 2'] == ''
                    or str(row['Value 2']).strip() == ''):
                value2 = None
            else:
                value2_str = str(row['Value 2'])
                # Handle special case where value contains "*mg/l" format
                if ('*mg/l' in value2_str.lower()
                        or '*mg/L' in value2_str.lower()):
        ...
        # Parse units and create Pyomo unit expressions for mathematical
        # operations. This enables unit-aware calculations and conversions.
        ...
        if value2 is not None and unit2:
            if unit2.lower() == 'mass fraction':
                ...
                # For mole fraction, multiply by the fraction to get actual
                # moles.
                expression = expression * value2
            else:
        ...
        try:
            ...
            if 'water' in category and not 'wastewater' in category:
                ...
                # PrOMMiS gives water density as 1 kg/L, so using 1 kg/L here
                # allows for consistency/compatibility.
            ...
            # This enables consistent mass-based LCA analysis
                ...
                if molar_mass is not None:
                    # Molar mass is in g/mol, so we need to convert to kg/mol
                    # by dividing by 1000.
...
def get_molar_mass(compound_name):
    """
    Helper function to retrieve the molar mass of a compound using PubChem API.

    This function searches the PubChem database for chemical compounds and
    calculates their molecular weights using the molecular formula.
    It provides automatic molecular weight lookups for converting molar flows
    to mass flows in LCA analysis.

    Parameters
    ----------
    compound_name : str
        Name of the compound to look up (e.g., 'Carbon dioxide', or
        'Sulfuric acid').
...
def parse_unit_to_pyomo(unit_str):
    """
    Helper function to parse and convert unit string to Pyomo unit expression.

    This function parses unit strings and converts them to Pyomo unit
    expressions that can be used for unit conversions and calculations.
    It handles complex unit combinations including multiplication, division,
    and exponents.
    ...
    pyomo.environ.units.Unit or None
        Pyomo unit expression if successfully parsed, None for mass/mole
        fractions or empty strings

    Notes
    -----
    -   Returns None for mass/mole fractions and empty strings
    -   Recognizes these fraction keywords: 'mass fraction', 'mole fraction',
        'mol fraction', 'mass frac', 'mole frac', 'mol frac'
    ...
    # Replace all '**' with '^' so that we can split the unit string by '*' for
    # multiplication
    unit_str = unit_str.replace('**', '^')

    # Split the unit string into a list of strings being multiplied, then into
    # a list of strings being divided
    unit_strings = unit_str.split('*')

    # The first substring will be used to initialize the total_unit
    first_unit = True
...
def get_unit(string, default=None):
    """
    Helper function to resolve individual unit strings to Pyomo unit objects.

    This function attempts to convert unit strings to Pyomo unit objects by
    trying various parsing strategies. It handles different unit formats
    including those with exponents and provides fallback options for
    unrecognized units.
    ...
    pyomo.environ.units.Unit or any
        Pyomo unit object if successfully resolved, default value if resolution
        fails.
    ...
    -   Automatically adds '**' for numeric exponents (e.g., 'm3' becomes
        'm**3')
    ...
    The function prints detailed error messages to help identify unit parsing issues, including:
    ...
    try:
        ...
        print(
            'Make sure you check that the unit string is correct, including '
            'capitalization and exponents, and that there are no parentheses.'
        )
        return default
