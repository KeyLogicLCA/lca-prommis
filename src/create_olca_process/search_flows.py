...
def _flowtype_from_string(s: str):
    ...
    if olca is None:
        raise ImportError("The 'olca' package is required but could not be imported.")
    if not s:
...
def search_and_select(
    ...
    Returns:
        (flow_uuid, process_uuid)  -- process_uuid will be None for elementary flows or if user aborts.
    """
